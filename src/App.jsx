...
function Login({ onLogin }) {
  ...
  const handleLogin = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const user = MOCK_USERS.find(u => u.email === emailLimpo && u.senha === senha);
      if (user) {
        resetarRateLimit(emailLimpo);
        const sessao = criarSessao(user);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (!s) {
        ...
        // Aviso 5 min antes de expirar por inatividade
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", background: "#F8FAFC" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <Topbar title={title} subtitle={subtitle} user={user} onLogout={() => { registrarAuditoria(sessao, ACOES.LOGOUT, {}); encerrarSessao(); setUser(null); setSessao(null); }} />
        {sessaoAviso && (
          <div style={{

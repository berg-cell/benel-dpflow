...
function Login({ onLogin }) {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      else { setErro("E-mail ou senha inválidos."); setLoading(false); }
    }, 600);
  };
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420, padding: "0 20px" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <Button onClick={handleLogin} disabled={loading} size="lg" style={{ marginTop: 4, width: "100%" }}>
...
// ─── AUDITORIA ────────────────────────────────────────────────────────────────
function Auditoria({ solicitacoes }) {
  const logs = solicitacoes.flatMap(s =>
    (s.historico || []).map(h => ({ ...h, sol_id: s.id, tipo: s.tipo }))
  ).sort((a, b) => b.sol_id - a.sol_id);

  const ACAO_COLOR = {
    criado: "#3B82F6", aprovado_gestor: "#10B981", aprovado_superior: "#8B5CF6",
    aprovado_dp: "#059669", devolvido: "#F97316", rejeitado: "#EF4444",
  };

  return (
    <div style={{ padding: 28 }}>
      <h2 style={{ margin: "0 0 20px", fontSize: 18, fontWeight: 700, color: "#111827" }}>Log de Auditoria</h2>
      <Card style={{ padding: 0 }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#F9FAFB" }}>
              {["Solicitação", "Tipo", "Ação", "Usuário", "Data/Hora", "Observação"].map(h => (
                <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "#6B7280", textTransform: "uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {logs.map((l, i) => (
              <tr key={i} style={{ borderTop: "1px solid #F3F4F6" }}>
                <td style={{ padding: "10px 16px", fontSize: 12, color: "#6B7280" }}>#{l.sol_id}</td>
                <td style={{ padding: "10px 16px", fontSize: 12, color: "#374151" }}>{l.tipo}</td>
                <td style={{ padding: "10px 16px" }}>
                  <span style={{
                    padding: "2px 10px", borderRadius: 10, fontSize: 11, fontWeight: 600,
                    background: (ACAO_COLOR[l.acao] || "#6B7280") + "22",
                    color: ACAO_COLOR[l.acao] || "#6B7280"
                  }}>{l.acao.replace(/_/g, " ")}</span>
                </td>
                <td style={{ padding: "10px 16px", fontSize: 12, fontWeight: 600, color: "#111827" }}>{l.usuario}</td>
                <td style={{ padding: "10px 16px", fontSize: 11, color: "#6B7280" }}>{l.data}</td>
                <td style={{ padding: "10px 16px", fontSize: 11, color: "#EF4444", fontStyle: l.obs ? "italic" : "normal" }}>{l.obs || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", background: "#F8FAFC" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <Topbar title={title} subtitle={subtitle} user={user} onLogout={() => setUser(null)} />
        <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          {page === "auditoria"         && <Auditoria solicitacoes={solicitacoes} blocos={blocos} />}
        </div>
      </div>
    </div>
  );
